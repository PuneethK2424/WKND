document.addEventListener('DOMContentLoaded', function () {
    ...
    function getPlaylistsApi() {
        return fetch('/libs/granite/csrf/token.json')
            .then(res => res.json())
            .then(csrf => {
                return fetch('/bin/wknd/abbvie-playlist.playlist-names.json', {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'CSRF-Token': csrf.token,
    ...
    function addVideoToPlaylists(payload) {
        fetch('/libs/granite/csrf/token.json')
            .then(res => res.json())
            .then(csrf => {
                fetch('/bin/wknd/abbvie-playlist/send-data-to-author.json', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'CSRF-Token': csrf.token,
    ...
    function createPlaylistApi(payload) {
        fetch('/libs/granite/csrf/token.json')
            .then(res => res.json())
            .then(csrf => {
                fetch('/bin/wknd/abbvie-playlist.create-playlist.json', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'CSRF-Token': csrf.token,
